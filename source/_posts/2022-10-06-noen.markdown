...
总体来说， Burst的 Neon intrinsics 还是一个比较低阶的实现， 绝大多数指令都没有相关的实现， 如果真的使用 Burst来写复杂的算法， 实现的实现会出现各种掣肘， 希望在之后的Burst的版本能快速的完善。 Unity之所以能做到支持Neon, 本质原因还是因为c#是支持JIT（ Just-in-Time ）， c# 代码不是一步到位编译成机器码。
