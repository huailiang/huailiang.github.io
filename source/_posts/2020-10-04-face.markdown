...
其中 $I$ 表示像素索引，$M$ 是投影的人脸区域，$ || \cdot || $表示 $l_2$距离，$A$是一个基于皮肤颜色的attention mask。
...
$$
L_{coef}(\chi) = \omega_\alpha || \alpha||^2+\omega_\beta|| \beta||^2+\omega_\sigma||\sigma||^2
$$
...
$$
L_{tex}(\chi) = \sum_{c\in{r,g,b} }var (\Tau_{c,R}(\chi))
$$
...
使用深度学习网络作用于任意张图片并不是一个简单的事情，在本文中，作者从单张图片人脸重建的结果学习一个置信度(反映重建质量)，并使用这个置信度收集人脸形状信息。具体来说，作者针对人脸形状参数[公式]生成一个反映置信度的向量[公式]
...
为了在图片集中训练C-Net，首先从图片${I^j}$得到人脸系数${\widehat{\chi}_j}$，其中$\widehat{\chi}_j=(\alpha_{aggr}, \beta^j, \sigma^j,\gamma^j, p^j)$，然后生成重建图片${I^{j'}}$，则损失函数定义为

$$
\zeta(\{ \widehat{\chi}_j\} ) = \frac{1}{M} \sum_{J=1}^{M} L(\widehat{\chi} ^j)
$$
...
__R-Net__ 的训练集来自CelebA, 300W-LP, I-JBA, LFW和LS3D数据集，并对其做了删选以得到平衡的人脸姿态和种族分布，最终用于训练的有大约26万张图片。然后再对图片进行对齐。网络的输入大小为[公式]，使用ImageNet预训练的权重作为初始，优化器为Adam，batch size是5，初始学习率为[公式]，一共进行了50万次迭代。

__C-Net__ 的训练集来自300W-LP, Multi-PIE和部分人脸识别数据集，对于300W-LP和Multi-PIE数据集，作者为每个人挑选了5张人脸角度分布均匀的图片，对于人脸识别数据集，对每个人随机挑选了5张图片，总的训练集包含大约1万个个体的5万张图片。作者固定住R-Net的权重，随机初始化C-Net的权重(除了最后一个全连接层初始化为0)，优化器为Adam，batch size同样为5，初始学习率为[公式]，一共进行1万次迭代。
