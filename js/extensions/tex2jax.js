MathJax.Extension.tex2jax = {
  ...
  PreProcess: function (element) {
    if (!this.configured) {
      ...
      if (typeof (this.config.previewTeX) !== "undefined" && !this.config.previewTeX) { this.config.preview = "none" } 
  ...
  endPattern: function (end) { return new RegExp(this.patternQuote(end) + "|\\\\.|[{}]", "g"); },
  ...
  scanElement: function (element, stop, ignore) {
    ...
    while (element && element != stop) {
      if (element.nodeName.toLowerCase() === '#text') {
        ...
        if (typeof (cname) !== "string") { cname = String(cname) } 
  ...
  startMatch: function (match, element) {
    ...
    } else if (match[0].substr(0, 6) === "\\begin") {   this.search = {
