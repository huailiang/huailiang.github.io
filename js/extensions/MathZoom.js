(function (HUB, HTML, AJAX, HTMLCSS, nMML) {
  ...
  var ZOOM = MathJax.Extension.MathZoom = {
    ...
    Zoom: function (event, math) {
      ...
      if (this.msiePositionBug) {
        if (this.msieSizeBug) { zoom.style.height = bbox.zH + "px"; zoom.style.width = bbox.zW + "px" } 
        if (zoom.offsetHeight > Mh) { zoom.style.height = Mh + "px"; zoom.style.width = (bbox.zW + this.scrollSize) + "px" }  
        if (zoom.offsetWidth > Mw) { zoom.style.width = Mw + "px"; zoom.style.height = (bbox.zH + this.scrollSize) + "px" }
      }
      if (this.operaPositionBug) { zoom.style.width = Math.min(Mw, bbox.zW) + "px" }  
    ...
    Position: function (zoom, bbox) {
      ...
      if (!ZOOM.msiePositionBug) { ZOOM.SetWH() } 
    },
  ...
  HUB.Browser.Select({
    MSIE: function (browser) {
      ...
      if (document.compatMode === "BackCompat") { ZOOM.scrollSize = 52 } 
