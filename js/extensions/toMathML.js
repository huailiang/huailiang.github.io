MathJax.Hub.Register.LoadHook("[MathJax]/jax/element/mml/jax.js", function () {
  ...
  MML.mbase.Augment({
    ...
    toMathMLquote: function (string) {
      ...
      for (var i = 0, m = string.length; i < m; i++) {
        ...
        if (n < 0x20 || n > 0x7E) string[i] = "&#x" + n.toString(16).toUpperCase() + ";";
        else {
          var c = { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[string[i]];
          if (c) { string[i] = c }
        }
  ...
  MML.chars.Augment({toMathML: function (space) { return (space || "") + this.toMathMLquote(this.toString()) }});
  MML.entity.Augment({toMathML: function (space) { return (space || "") + "&" + this.data[0] + ";<!-- " + this.toString() + " -->" }});
  MML.xml.Augment({toMathML: function (space) { return (space || "") + this.toString() }});
  MathJax.Hub.Register.StartupHook("TeX mathchoice Ready", function () {MML.TeXmathchoice.Augment({toMathML: function (space) { return this.Core().toMathML(space)}});});
