(function (TEX, HUB, AJAX) {
    ...
    Push: function () {
      ...
      for (i = 0, m = arguments.length; i < m; i++) {
        ...
        if (top instanceof Array) { this.Pop(); this.Push.apply(this, top) }
          ...
          this.data.push(item);
          if (item.env) {
            for (var id in this.env) { if (this.env.hasOwnProperty(id)) { item.env[id] = this.env[id] } }
            this.env = item.env;
  ...
  STACKITEM.subsup = STACKITEM.Subclass({
    ...
    checkItem: function (item) {
      ...
      if (this.SUPER(arguments).checkItem.call(this, item)) { TEX.Error("Missing open brace for " + script) }
    },
    Pop: function () { }
  });
  ...
  STACKITEM.begin = STACKITEM.Subclass({
    ...
    checkItem: function (item) {
      if (item.type === "end") {
        if (item.name !== this.name) { TEX.Error("\\begin{" + this.name + "} ended with \\end{" + item.name + "}") }
      ...
      for (var id in src) {
        if (src.hasOwnProperty(id)) {
          if (typeof src[id] === 'object' && !(src[id] instanceof Array) &&
            (typeof dst[id] === 'object' || typeof dst[id] === 'function')) { this.Add(src[id], dst[id], src[id], nouser) }
  ...
  var PARSE = MathJax.Object.Subclass({
    ...
    Prime: function (c) {
      ...
      if (base.type === "msubsup" && base.data[base.sup]) { TEX.Error("Prime causes double exponent: use braces to clarify") }
      var sup = ""; this.i--;
    ...
    mi2mo: function (mi) {
      ...
      for (id in mo.defaults) { if (mo.defaults.hasOwnProperty(id) && mi[id] != null) { mo[id] = mi[id] } }
      for (id in MML.copyAttributes) { if (MML.copyAttributes.hasOwnProperty(id) && mi[id] != null) { mo[id] = mi[id] } }
      return mo;
    },
    Comment: function (c) {
    ...
    MmlToken: function (name) {
      ...
      while (attr !== "") {
        ...
        if (!MML[type].prototype.defaults[match[1]] && !this.MmlTokenAllow[match[1]]) { TEX.Error(match[1] + " is not a recognized attribute for " + type) }
    ...
    Rule: function (name, style) {
      ...
      if (style !== 'blank') {
        if (parseFloat(w) && parseFloat(h) + parseFloat(d)) { def.mathbackground = (this.stack.env.color || "black") }
    ...
    Begin: function (name) {
      ...
      if (++this.macroCount > TEX.config.MAXMACROS) { TEX.Error("MathJax maximum substitution count exceeded; is there a recursive latex environment?") }
    ...
    GetDimen: function (name) {
      ...
      if (this.string.charAt(this.i) == '{') {
        ...
        if (dimen.match(/^\s*([-+]?(\.\d+|\d+(\.\d*)?))\s*(pt|em|ex|mu|px|mm|cm|in|pc)\s*$/)) { return dimen.replace(/ /g, "") }
      } else {
    ...
    SubstituteArgs: function (args, string) {
      ...
      while (i < string.length) {
        ...
        if (c === "\\") { text += c + string.charAt(i++) }
          ...
          c = string.charAt(i++);
          if (c === '#') { text += c } else {
            if (!c.match(/[1-9]/) || c > args.length) { TEX.Error("Illegal macro parameter reference") }
    ...
    AddArgs: function (s1, s2) {
      ...
      if (s1.length + s2.length > TEX.config.MAXBUFFER) { TEX.Error("MathJax internal buffer size exceeded; is there a recursive macro call?") }
      return s1 + s2;
    }
  });
  TEX.Augment({
