(function (AJAX, HUB, HTMLCSS) {
  ...
  var FONTTEST = MathJax.Object.Subclass({
    ...
    testFont: function (font) {
      ...
      if (W) {
        ...
        if (this.div.offsetWidth != W[3] || this.div.offsetHeight != W[4]) {
          ...
          for (var i = 0, m = this.testSize.length; i < m; i++) { if (this.testStyleChar(font, this.testSize[i])) { return true } }
        }
      }
      return false;
    },
    ...
    getComparisonWidths: function (string, noStyleChar) {
      if (HTMLCSS.FONTDATA && HTMLCSS.FONTDATA.hasStyleChar && !noStyleChar) { string += this.styleChar + " " + this.compChar }
    ...
    loadComplete: function (font, n, done, status) {
      ...
      if (HUB.Browser.isFirefox && HTMLCSS.allowWebFonts) {
        ...
        host += "/";
        if (AJAX.fileURL(HTMLCSS.webfontDir).substr(0, host.length) !== host) { this.firefoxFontError(font) }
      }
  ...
  HTMLCSS.Augment({
    ...
    Config: function () {
      ...
      if (settings.font && settings.font !== "Auto") {
        if (settings.font === "TeX (local)") { this.config.availableFonts = ["TeX"]; this.config.preferredFont = "TeX"; this.config.webFont = "TeX" }
        else if (settings.font === "STIX (local)") { this.config.availableFonts = ["STIX"]; this.config.preferredFont = "STIX"; this.config.webFont = "TeX" }
    ...
    removeSTIXfonts: function (fonts) {
      for (var i = 0, m = fonts.length; i < m; i++) { if (fonts[i] === "STIX") { fonts.splice(i, 1); m--; i--; } }
    ...
    preTranslate: function (state) {
      ...
      for (i = 0; i < m; i++) {
        ...
        if (prev && String(prev.className).match(/^MathJax(_Display)?$/)) { prev.parentNode.removeChild(prev) }
    ...
    initFont: function (name) {
      ...
      if (AVAIL && AVAIL.length && HTMLCSS.Font.testFont(FONTS[name])) { FONTS[name].available = true; return null }
    ...
    createSpace: function (span, h, d, w, color) {
      ...
      if (w >= 0) {
        ...
        if (HTMLCSS.safariNegativeSpaceBug && span.parentNode.firstChild == span) { this.createBlank(span, 0, true) }
      }
    ...
    positionDelimiter: function (span, h) {
      ...
      if (h) {
        if (this.safariVerticalAlignBug || this.konquerorVerticalAlignBug ||
          ...
          if (span.firstChild.style.display === "" && span.style.top !== "") { span = span.firstChild; h -= parseFloat(span.style.top) }
        ...
        if (n >= 0xD800 && n < 0xDBFF) {
          ...
          for (id = 0, M = RANGES.length; id < M; id++) {
            ...
            if (N && n >= RANGES[id].low && n <= RANGES[id].high) {
              ...
              if (variant["variant" + RANGES[id].offset]) { variant = this.FONTDATA.VARIANT[variant["variant" + RANGES[id].offset]] }
              break;
            }
          }
        }
    ...
    lookupChar: function (variant, n) {
      ...
      for (i = 0, m = variant.FONTS.length; i < m; i++) {
        ...
        if (font[n]) {
          ...
          if (HTMLCSS.allowWebFonts && !font.available) { this.loadWebFont(font) } else { return font }
      ...
      if (font.Ranges) {
        for (var i = 0, m = font.Ranges.length; i < m; i++) {
          ...
          if (c <= font.Ranges[i][1]) {
            ...
            for (var j = font.Ranges.length - 1; j >= 0; j--) { if (font.Ranges[j][2] == file) { font.Ranges.splice(j, 1) } }
  ...
  MathJax.Hub.Register.StartupHook("mml Jax Ready", function () {
    ...
    MML.mbase.Augment({
      toHTML: function (span) {
        ...
        for (var i = 0, m = this.data.length; i < m; i++) { if (this.data[i]) { this.data[i].toHTML(span) } }
      ...
      HTMLcomputeBBox: function (span, full, i, m) {
        ...
        while (i < m) {
          ...
          if (!full && core.HTMLcanStretch("Vertical")) { stretchy.push(core); core = (core.CoreMO() || core) }
      ...
      HTMLcleanBBox: function (BBOX) {
        if (BBOX.h === this.BIGDIMEN) { BBOX.h = BBOX.d = BBOX.H = BBOX.D = BBOX.w = BBOX.rw = BBOX.lw = 0 }
      ...
      HTMLhandleColor: function (span) {
        ...
        if ((values.mathbackground && values.mathbackground !== MML.COLOR.TRANSPARENT) ||
          ...
          if (HTMLCSS.msieInlineBlockAlignBug) {
            ...
            if (borders && HTMLCSS.msieBorderWidthBug) { H += borders.top + borders.bottom; W += borders.left + borders.right }
      ...
      HTMLhandleSpace: function (span) {
        if (this.useMMLspacing) {
          ...
          if (values.scriptlevel <= 0 || this.hasValue("lspace") || this.hasValue("rspace")) {
            ...
            while (parent && parent.isEmbellished() && parent.Core() === core) { core = parent; parent = parent.Parent(); span = core.HTMLspanElement() }
    ...
    MML.mi.Augment({
      toHTML: function (span) {
        ...
        for (var i = 0, m = this.data.length; i < m; i++) { if (this.data[i]) { this.data[i].toHTML(span, variant) } }
    ...
    MML.mo.Augment({
      toHTML: function (span) {
        ...
        if (values.largeop) { variant = HTMLCSS.FONTDATA.VARIANT[values.displaystyle ? "-largeOp" : "-smallOp"] }
        ...
        if (isScript && HTMLCSS.fontInUse === "STIX" && text.match(/['`"\u00B4\u2032-\u2037]/)) { variant = HTMLCSS.FONTDATA.VARIANT["-STIX-variant"] }
        for (var i = 0, m = this.data.length; i < m; i++) { if (this.data[i]) { this.data[i].toHTML(span, variant, this.HTMLremap, mapchars) } }
        if (!span.bbox) { span.bbox = { w: 0, h: 0, d: 0, rw: 0, lw: 0 } }
      ...
      CoreText: function (parent) {
        ...
        while (parent.isa(MML.mrow) && parent.data.length === 1 && parent.data[0]) { parent = parent.data[0] }
    ...
    MML.mphantom.Augment({
      toHTML: function (span, HW, D) {
        ...
        if (this.data[0] != null) {
          ...
          for (var i = 0, m = span.childNodes.length; i < m; i++) { span.childNodes[i].style.visibility = "hidden" }
        }
        this.HTMLhandleSpace(span);
        this.HTMLhandleColor(span);
        return span;
    ...
    MML.mpadded.Augment({
      ...
      HTMLlength2em: function (span, length, mu, d, m) {
        ...
        if (d && String(length).match(/^\s*[-+]/)) { return Math.max(m, span.bbox[d] + v) } else { return v }
      },
      HTMLstretchH: MML.mbase.HTMLstretchH,
      HTMLstretchV: MML.mbase.HTMLstretchV
    });
    ...
    MML.munderover.Augment({
      toHTML: function (span, HW, D) {
        ...
        if (!values.displaystyle && this.data[this.base] != null &&
          this.data[this.base].CoreMO().Get("movablelimits")) { return MML.msubsup.prototype.toHTML.call(this, span) }
        ...
        for (i = 0, m = this.data.length; i < m; i++) {
          if (this.data[i] != null) {
            ...
            if (accent && box.bbox.w <= 1 / HTMLCSS.em + .0001) { // images can get the width off by 1px
              ...
              if (box.bbox.lw && !HTMLCSS.zeroWidthBug) { box.insertBefore(HTMLCSS.createSpace(box.parentNode, 0, 0, -box.bbox.lw), box.firstChild) }
    ...
    MML.msubsup.Augment({
      toHTML: function (span, HW, D) {
        ...
        var sup, sub;
        if (this.HTMLnotEmpty(this.data[this.sup])) { sup = HTMLCSS.createBox(stack); children.push(this.data[this.sup].toHTML(sup)) }
        if (this.HTMLnotEmpty(this.data[this.sub])) { sub = HTMLCSS.createBox(stack); children.push(this.data[this.sub].toHTML(sub)) }
        HTMLCSS.MeasureSpans(children);
